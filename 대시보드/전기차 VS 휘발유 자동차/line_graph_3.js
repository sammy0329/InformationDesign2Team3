...
Cost.prototype.line = function (scale) {
  ...
  linePath  = lineGroup
    ...
    .attr("d", line(this.cost_data)) // use the value of myline(xy) as the data, 'd'
    .style("fill", "none")
    .style("stroke", this.color)
    .style("stroke-width", 2)
    .attr("transform","translate("+x+130+", 0)");



  d3.select("svg").append("defs").append("clipPath").attr("id","clip")
		.append("rect").attr("width",width + margin.left + margin.right).attr("height",height);
...
function axis(ev,ice) {
  ...
  xAxisGroup = svg.append("g").attr("id", "xAxisG").attr("transform","translate("+x+130+","+height+")").call(xAxis);

  // svg.append("g").attr("id", "xAxisG").attr("transform","translate(20, "+(line_height-100)+")").call(xAxis);
  // svg.append("g").attr("id", "xAxisG").attr("transform","translate(0,0)").call(xAxis);

  let yAxis = d3
    .axisLeft(yScale)
    .tickSize(-width);

  yAxisGroup = d3.select("svg").append("g").attr("id", "yAxisG").attr("transform","translate("+x+130+", 0)").call(yAxis);
 
  svg.append("text").text("유지비")
			.attr("x",0-height/2)
			.attr("y",0)
			.attr("dy","1em")
			.style("text-anchor", "middle")
			.attr("transform","rotate(-90)");
		svg.append("text").text("주행거리")
			.attr("x",(width/2)+130)
  ...
  var zoom = d3.zoom()
					...
					.translateExtent([[0,-height],[2*width,2*height]])//the area top left to bottom right
					.on("zoom",zoomed);
